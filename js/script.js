...
$(function () {
	...
	// 		} else {
	// 			// rankingTop10(global_rakingTop10Type); // 
    ...
    if(typeof advancedSearchFormSelected === 'function') {
...
/* check_member_opt_in_policy
========================================================================== */
function check_member_opt_in_policy() {
	if ($('#fs_CheckoutSuccess').length) {
		...
		if (memberId != 'guest') {
			...
			if (response['result'] == false || response['is_opt_in'] == '') {
				...
				const postUrl = 'https://h15yyu8zof.execute-api.ap-northeast-1.amazonaws.com/prod/post_member_opt_in_policy';
...
/* advancedSearchForm (複数コンテナ無限拡張・完全同期・セキュア版)
========================================================================== */

function advancedSearchForm() {
    
    var $masterForm = $('#header-keywordSearch .advancedSearchForm').first();
    if (!$masterForm.length) return;

    // 0. ヘッダーの大元フォームに、同期の基準となるデータ属性(data-base-name)を付与
    $masterForm.find('input').each(function() {
    ...
    // ==========================================
    // 1. フォームの動的生成（共通クローン処理）
    // ==========================================
    // DRY原則に基づき、クローン処理を一元化
    function cloneAndSetupForm(targetSelector, prefix) {
        if ($(targetSelector).length) {
            $(targetSelector).each(function(index) {
                var $clonedForm = $masterForm.clone(false);
                
                // 不要なUIパーツと重複するとバグの原因になるID・for属性を削除
                $clonedForm.find('.formClose, .close, .advancedSearchFormTitle').remove();
                $clonedForm.find('[id]').removeAttr('id');
                $clonedForm.find('label[for]').removeAttr('for');
                
                // フォーム同士が干渉しないよう、name属性にプレフィックスとインデックスを付けて完全に独立させる
                $clonedForm.find('input').each(function() {
                    var baseName = $(this).attr('data-base-name');
                    if (baseName) {
                        $(this).attr('name', prefix + '_' + index + '_' + baseName);
                    }
                });
                ...
                $(this).html($clonedForm);
            });
        }
    }

    // ボディ用とスライドナビ用のコンテナへクローンを展開
    cloneAndSetupForm('.body-advanced-search-container', 'body');
    cloneAndSetupForm('.slide-navi-advanced-search-container', 'slide');

    // ==========================================
    // 2. フォームの開閉ロジック（ヘッダー側）
    // ==========================================
    $(document).on('click', '#header-keywordSearch .keywordSearch', function () {
        if ($masterForm.css('display') === 'none') {
            $masterForm.css('display', 'flex').hide().slideDown();
            $('#globalNavi-overlay').fadeIn(200);
        }
    });

    // イベントの多重登録をリセットし、バブリングを遮断して正確に開閉
    ...
    $(document).on('click', '.advancedSearchForm .fs-c-radio__label, .advancedSearchForm .fs-c-checkbox__label, .advancedSearchForm input[type="radio"], .advancedSearchForm input[type="checkbox"]', function (e) {
        ...
        // data-base-name を使って、ページ内の全てのフォーム（ヘッダー・各ボディ群）をまとめて指定
...
function advancedSearchFormURL() {
    ...
    formType.each(function () {
        // [Security] タグの値もエンコード
        tags += ',' + encodeURIComponent($(this).val());
    });
    ...
    if (priceRange != undefined) {
        ...
        var minprice = encodeURIComponent(priceRange_ary[0]);
        var maxprice = priceRange_ary[1] ? encodeURIComponent(priceRange_ary[1]) : '';
    ...
    if ($('.fs_ProductSearch, #fs_ProductSearch').length || window.location.pathname.indexOf('/p/search') > -1) {
        ...
        for (var i = 0; i < searchUrl_ary.length; i++) {
            if (searchUrl_ary[i].indexOf('keyword=') != -1) {
                ...
                $('.fs-p-searchForm__input').val(keyword);
            } else if (searchUrl_ary[i].indexOf('tag=') != -1) {
                tags = searchUrl_ary[i].replace('tag=', '');
            } else if (searchUrl_ary[i].indexOf('minprice=') > -1) {
                priceRangeVal = searchUrl_ary[i].replace('minprice=', '');
            } else if (searchUrl_ary[i].indexOf('maxprice=') > -1) {
...
function initAdvancedSearchRealtimePreview() {
    ...
    function fetchRealtimePreview() {
        ...
        fetch(`${API_URL}?key=${encodeURIComponent(searchKey)}`)
            ...
            .catch(error => {
                console.error('[RealtimeSearch Error]', error);
    ...
    function renderPreview(totalCount, items) {
        ...
        if (totalCount > 0 && items && items.length > 0) {
            items.forEach(item => {
                ...
                if (['tl1', 'tl2', 'tl3'].includes(seriesCode)) seriesCode = 'tl';
                else if (['ona', 'obk'].includes(seriesCode)) seriesCode = 'of2';
                else if (seriesCode === 'gbp') seriesCode = 'gbt';

                // [Security] 詳細URL構築時にもエンコードを適用
                const detailUrl = `/c/series/${encodeURIComponent(seriesCode)}/${encodeURIComponent(productCode)}`;
...
/* product_detail_ncm_contents_banner
========================================================================== */
function product_detail_ncm_contents_banner() {
	if ($('#fs_ProductDetails').length) {
		var url = window.location.pathname.substring(1);
		url = url.split('/');
		var series = url[url.length - 1].split('-');
...
function sns_post_list_product_detail_10() {
	if ($('#fs_ProductDetails #content_shirai_fan.product_detail').length) {
		...
		const url = 'https://h15yyu8zof.execute-api.ap-northeast-1.amazonaws.com/prod/get_add_data_v2';
		...
		$.ajax({
			...
			success: function (response) {
				...
				for (let i = 0; i < sns_modal_data.length; i++) {
					...
					list_html += `<li class="modal-open" data-target="post_modal" data-postid="${post_id}">
						<img src="${thumbnail_url}" style="width:${size_adjustment}%;height:${size_adjustment}%;" alt="sns_post_${post_id}">
					</li>`;

					if (i === 9 && $('#fs_ProductDetails').length) break;
				}
				...
				if (sns_modal_data.length > 10) {
					...
					$('#posted_list').after(button_html);

					$('.fs-c-buttonContainer.more-button').on('click', function () {
						sns_post_list_product_detail_all();
						$(this).hide();
					});
				}
...
/* sns_post_list
========================================================================== */
function sns_post_list_product_detail_all() {
	if ($('#fs_ProductDetails #content_shirai_fan.product_detail').length) {
		// 現在のページURLの最後のスラッシュ以降を取得
		const lastPart = location.pathname.substring(location.pathname.lastIndexOf('/') + 1);
		...
		if (data != undefined && data != '') {
			...
			for (var i in data) {
				var sku_no = data[i].sku_no.toLowerCase(),
					...
					name = data[i].name,
					selling_price = Number(data[i].selling_price),
					normal_price = Number(data[i].normal_price),
					icon = data[i].icon,
					size = data[i].size,
					product_image_group = Math.floor(id / 100),
...
function multipleReviewList() {
	...
	if (url_ary[0] == 'https://shirai-store.net/f/reviewList') {
		...
		function get_reviews(params_review){
			...
			reviews = sanitizeString(reviews)
			// console.log(reviews)
			...
			// 次のページのexclusive_start_keyを取得
			// console.log('最初のnextStartKey:',JSON.stringify(nextStartKey))
			// console.log('最初のprevStartKey:',JSON.stringify(prevStartKey))

			// console.log('currentPage:',currentPage)
			// console.log('startKeyAryNumber:',currentPage - 1)

			// console.log('startKeyAry_A:',startKeyAry)
			// console.log('prevStartKey_A:',prevStartKey);
...
// ランキングが10位以内のみ表示する版
async function productListAddData() {
  if (!($('#fs_ProductCategory').length || $('#fs_ProductSearch').length || $('#fs_CouponProductList').length)) return;
  ...
  const toHalfStep = (score) => {
    ...
    else if (s == 5) return '5.0';
    return '0';
  };

  // URL末尾モデルコードをSKUとして大文字で収集
  const skuList = [];
  $('.fs-c-productListItem').each(function () {
  ...
  try {
    ...
    console.error('productListAddData API error:', e);
    return;
  }

  // {SKU(大文字) -> item} マップ化（数値項目を変換）
  const map = {};
  ...
  $('.fs-c-productListItem').each(function () {
    ...
    if ($row.find('.mark-catRank').length === 0) {
      ...
      if (catName && rank > 0 && rank <= 10) {
        const rankingHTML = '<span class="mark-catRank">' + catName + ' ' + rank + '位</span>';
        $row.find('.fs-c-productName').before(rankingHTML);
      }
    }

    // 2) 価格（normal_price > 現在の表示価格 の場合のみ旧価格を出す）
    const $selling = $row.find('.fs-c-productPrice--selling').first();
    if ($selling.length && $row.find('.js-added-normal-price').length === 0) {
      const current = toInt($selling.find('.fs-c-price__value').first().text());
      const normal = data.normal_price;
      if (normal > current && current > 0) {
        const normalPriceHTML =
          '<div class="fs-c-productPrice fs-c-productPrice--selling js-added-normal-price">' +
            '<span class="fs-c-productPrice__main">' +
              '<span class="fs-c-productPrice__main__price fs-c-price">' +
                '<span class="fs-c-price__currencyMark">¥</span> ' +
                '<span class="fs-c-price__value">' + formatYen(normal) + '</span>' +
              '</span>' +
            '</span> ' +
            '<span class="fs-c-productPrice__addon"><span class="fs-c-productPrice__addon__label">税込</span></span>' +
          '</div>';
        $selling.addClass('salePrice');
        $selling.before(normalPriceHTML);
      }
    }

    // 3) レビュー（.fs-c-productPrices の前）
    if ($row.find('.fs-c-reviewStars').length === 0) {
      const score = toHalfStep(data.average_rating);
      const count = data.number_review;
      if (score !== '0') {
        const href = $row.find('.fs-c-productName > a').attr('href') || '';
        const modelCode = href.split('/').pop();
        const reviewHTML =
          '<div class="fs-c-rating__stars fs-c-reviewStars" data-ratingcount="' + score + '">' +
            '<a href="/f/reviewList?modelCode=' + modelCode + '">（' + count + '）</a>' +
          '</div>';
        $row.find('.fs-c-productPrices').before(reviewHTML);
      }
    }
  });
}
...
function displayEstimatedDeliveryDate() {
        ...
        <dd>
            ...
            <div>
                ...
                <p style="margin: 0; font-size: 12px; color: #777; line-height: 1.4; text-align: left;">
                    ※在庫状況、実際の詳細な住所により変動する場合があります。<br>
    ...
    async function updateEstimatedDelivery() {
        ...
        // ★ 在庫切れの判定追加
        // -----------------------------------------------------
        const isOutOfStock = $('#productActionBox').hasClass('outOfStock') || $('.fs-c-productNotice--outOfStock').length > 0;
        if (isOutOfStock) {
            // 文字数が長いためサイズを調整し、注意色で表示
            $dateDisplay.text('在庫がないため表示できません').css({'font-size': '14px', 'color': '#d9534f'});
            return;
        } else {
            // 在庫がある場合は元のスタイルに戻す
            $dateDisplay.css({'font-size': '18px', 'color': '#e87909'});
        }
        // -----------------------------------------------------
    ...
    // セット商品の切り替えなど、その他のUI操作が行われた際にも再計算をフックする
    $(document).on('click', '.set-select-list li', function() {
        // DOMの更新（outOfStockの付与など）を待つため少し遅延させる
        setTimeout(updateEstimatedDelivery, 100);
    });

    // 初期描画処理 (郵便番号が保存されていれば計算)
    if ($('#zipInput').val() || $('#productActionBox').hasClass('outOfStock') || $('.fs-c-productNotice--outOfStock').length > 0) {
        updateEstimatedDelivery();
    }
}
...
function recommendTop10() {
  const itemsKey = 'recommend_item_10';

  $.ajax({
    url: 'https://h15yyu8zof.execute-api.ap-northeast-1.amazonaws.com/prod/get_add_data_v2',
    method: 'POST',
    contentType: 'application/json',
    dataType: 'json',
    data: JSON.stringify({ items: itemsKey })
  })
  .done(function (resp) {
    // 両対応：配列直返し or { data: [...] }
    ...
    // 念のため初期化（必要なければ削除OK）
    // $('.productTop10Slider.recommend ul').empty();

    for (var i = 0; i < list.length; i++) {
      var row = list[i];

      // ====== API -> 既存変数へマッピング ======
      var productUrl   = String(row.sku_no || '').toLowerCase();
      var seriesCode   = productUrl.slice(0, 3);
      var productId    = Number(row.id || 0);
      var productName  = row.name || '';
      var productId_12Len = zeroPadding(productId, 12);
      var product_image_group = Math.floor(productId / 100);

      var sellingPrice = Number(row.selling_price || 0);
      var normalPrice  = Number(row.normal_price  || 0);
      var icon         = row.icon || '';
      var size         = row.size || '';

      var reviewScore  = Number(row.average_rating || 0);
      var reviewCount  = Number(row.number_review  || 0);

      var thumbnail    = Number(row.thumbnail_number || 1);
      var categoryName = row.category_name || '';
      var categoryUrl  = row.category_url  || '';
      ...
      if (sellingPrice < normalPrice) {
        ...
        priceHtml =
          '<p class="priceBox"><span class="price">¥ ' +
          sellingPrice.toLocaleString() +
          '<span class="tax">(税込)</span></span></p>';
      }
      ...
      // アイコンHTML
      var iconHtml = '';
      var icon_ary = String(icon).split(',');
      for (var j = 0; j < icon_ary.length; j++) {
        if (!icon_ary[j]) continue;
        var kv = icon_ary[j].split(':');
        var key = kv[0], val = kv[1];

        if (key === 'mark-categoryRank' && Number(val) < 4) {
          var shortName = categoryNameShorter ? categoryNameShorter(categoryName) : categoryName;
          iconHtml += '<a href="/f/ranking-' + categoryUrl + '" class="mark-catRank">' + shortName + ' ' + val + '位</a>';
        }
        if (key === 'mark-new')            iconHtml += '<span class="mark-new">新着</span>';
        if (key === 'mark-limitedProduct') iconHtml += '<span class="mark-limitedProduct">当店限定商品</span>';
        if (key === 'mark-sale')           iconHtml += '<span class="mark-sale">SALE</span>';
      }

      // レビュー星の段階丸め
      var rf = reviewScore.toFixed(1);
      var reviewStars = 0;
      if (rf < 0.5)       reviewStars = '0';
      else if (rf < 1.0)  reviewStars = '0.5';
      else if (rf < 1.5)  reviewStars = '1.0';
      else if (rf < 2.0)  reviewStars = '1.5';
      else if (rf < 2.5)  reviewStars = '2.0';
      else if (rf < 3.0)  reviewStars = '2.5';
      else if (rf < 3.5)  reviewStars = '3.0';
      else if (rf < 4.0)  reviewStars = '3.5';
      else if (rf < 4.5)  reviewStars = '4.0';
      else if (rf < 5.0)  reviewStars = '4.5';
      else                reviewStars = '5.0';

      var reviewHTML = '';
      if (reviewStars !== '0') {
        reviewHTML = '<div class="fs-c-rating__stars fs-c-reviewStars" data-ratingcount="' + reviewStars + '"><a href="https://shirai-store.net/f/reviewList?modelCode=' + productUrl + '">（' + reviewCount + '）</a></div>';
      }
      ...
      var h =
        ...
        reviewHTML +
        '<a href="/c/series/' + seriesCode + '/' + productUrl + '">' + priceHtml + '</a></li>';
  ...
  .fail(function (xhr, status, err) {
    console.error('recommendTop10 API fetch failed:', status, err);
  });
}
...
function recommendList() {
  const itemsKey = 'recommend_item_50';
  ...
  .done(function (resp) {
    ...
    // ← ここがポイント：配列直返し or {data: [...]} の両対応
    var apiData = Array.isArray(resp) ? resp
                 : (resp && Array.isArray(resp.data)) ? resp.data
                 : [];
    ...
    for (var i = 0; i < apiData.length; i++) {
      ...
      var reviewFixed = reviewScore.toFixed(1);
      var reviewStars = 0;
      if (reviewFixed < 0.5)       reviewStars = '0';
      else if (reviewFixed < 1.0)  reviewStars = '0.5';
      else if (reviewFixed < 1.5)  reviewStars = '1.0';
      else if (reviewFixed < 2.0)  reviewStars = '1.5';
      else if (reviewFixed < 2.5)  reviewStars = '2.0';
      else if (reviewFixed < 3.0)  reviewStars = '2.5';
      else if (reviewFixed < 3.5)  reviewStars = '3.0';
      else if (reviewFixed < 4.0)  reviewStars = '3.5';
      else if (reviewFixed < 4.5)  reviewStars = '4.0';
      else if (reviewFixed < 5.0)  reviewStars = '4.5';
      else                         reviewStars = '5.0';
  ...
  .fail(function (xhr, status, err) {
    console.error('recommend API fetch failed:', status, err);
  });
}
...
/* adjust_anchor_scroll
ハッシュ付きURLを読み込んだ際に固定ヘッダーを考慮した位置に移動するようにする
========================================================================== */

function adjust_anchor_scroll() {
...
function scroll_to_target($target) {
    ...
    const offset_top = $target.offset().top - header_height;
